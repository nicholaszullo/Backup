...
typedef struct shared_mem{
	...
	struct cs1550_sem mutex;						//Only one can access shared resources at a time
}shared_mem; 
...
void openMuseum(){
	...
	int i;
	down(&(shared->mutex));							//Write to shared variables must be protected
	shared->guidesWaiting++;
	up(&(shared->mutex));							//Release lock before sleep to prevent deadlock
	down(&(shared->maxtwo));						//Only 2 guides at a time can open musuem 
	down(&(shared->mutex));							//Protect shared variable
	shared->guidesWaiting--;
	up(&(shared->mutex));
...
/**
 *		This is called when a visitor does not leave from the museum
 *		This is needed to not decrement numInMueum becuase the visitor does not leave from the musuem
 */
void visitorDoesNotTour(){
	struct timeval curr;
...
void tourMuseum(){
	...
	if (shared->maxten.value <=0 && (shared->visitorsArriving - (shared->guidesArriving*10)) >= 0 && shared->guidesWaiting == 0){		
		visitorDoesNotTour();										//If no guides waiting or arriving, and max has been reached dont enter deadlock												
		exit(0);													//Do not run visitorLeaves() in visitorArrives()
	}
...
void main(int argc, char *argv[]){
	...
	//Dont need to protect with mutex because only 1 process right now
	...
	shared->mutex.value = 1;						//1 can access at a time
		...
		switch (opt){
			...
			case 0:								//A more than 1 letter flag will be read as a 0, and temp is set to value in 4th column 
				...
				case 6:							//Random seed for tour guide arrival
					...
					break;
				default:
					printf("Unknown long argument provided! %c\n", optopt);
					break;
				}
				break;
	...
	munmap(shared, sizeof(shared_mem));			//Unallocate memory from mmap
}