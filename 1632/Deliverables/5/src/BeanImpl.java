...
public class BeanImpl implements Bean {
	...
	BeanImpl(int slotCount, boolean isLuck, Random rand) {
		...
		this.isLuck = isLuck;
		this.rand = rand;
		double skillAverage = (double) (slots - 1) * 0.5;
		double skillStd = (double) Math.sqrt(slots * 0.5 * (1 - 0.5));
		skillLevel = (int) Math.round(rand.nextGaussian() * skillStd + skillAverage);
		counter = skillLevel;
	}
