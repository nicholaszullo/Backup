...
int main(int argc, char* argv[])
{
    ...
    //A*B*C
	#pragma omp parallel for num_threads(NUM_THREADS) schedule(static, CHUNK_SIZE) default(none) shared(tempMatrix, matrixA, matrixB, matrixC, outputMatrix) private(i,j,k)
    for(i=0;i<NROW;i++){
        for(j=0; j<NCOL; j++){
            for(k=0;k<NROW;k++){
