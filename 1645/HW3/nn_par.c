...
int main() {
	...
	#pragma omp parallel for num_threads(NUM_THREADS) schedule(guided, CHUNK_SIZE) default(none) shared(pixels, dx, dy) private(i, j, x0, y0, x, y, count, tempx) 
    ...
    double totalSum = 0.0;
    double avg;
	#pragma omp parallel for num_threads(NUM_THREADS) schedule(guided, CHUNK_SIZE) default(none) shared(pixels) private(i, j) reduction(+:totalSum)
    for (i = 0; i < N; i++) {
        for (j = 0; j < N; j++) {
	...
	#pragma omp parallel for num_threads(NUM_THREADS) schedule(guided, CHUNK_SIZE) default(none) shared(pixels, avg) private(i,j)
    for (i = 0; i < N; i++) {
        for (j = 0; j < N; j++) {
