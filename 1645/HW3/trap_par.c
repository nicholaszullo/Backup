...
int main() {
	...
	#pragma omp parallel for num_threads(NUM_THREADS) schedule(guided, CHUNK_SIZE) default(none) shared(step_size) private(i, p_current, f_result_high, f_result_low) reduction(+:area)
    for(i = 0; i < NSTEPS; i++)
    {
