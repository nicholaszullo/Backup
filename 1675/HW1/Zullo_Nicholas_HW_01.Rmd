---
title: "CS 1675 Fall 2021 Homework: 01"
subtitle: "Assigned September 1, 2021; Due: September 8, 2021"
author: "Nicholas Zullo"
date: "Submission time: September 8, 2021 at 4:00PM EST"
...
If you need help with understanding the R syntax please see the [R4DS book](https://r4ds.had.co.nz/) and/or the R tutorial videos and demos available on the [Canvas site](https://canvas.pitt.edu/courses/108747/pages/introduction-to-r-tutorials).  
...
```{r, solution_01b}
iris %>% ggplot(mapping = aes(x = Sepal.Length)) + geom_histogram(color="black", bins=6) 
```
...
It seems that there are a lot of outlier diamonds that have a very high price. Despite the mean slowly increasing as the color letters move down the alphabet, the top outlier price remains the same. The difference between the 25th and 75th percentile increases as color letters increase as well. Also, the minimum price does not change as color changes, like the maximum. This is surprising because intuitively I would expect higher color diamonds to have a higher starting price.
...
```{r, solution_02f}
diamonds %>% filter(cut == "Ideal", clarity == "IF") %>% ggplot(mapping= aes(x=color, y=price)) + geom_boxplot(aes(fill=carat > median(diamonds$carat))) + facet_grid(clarity ~ cut)  + theme(legend.position = "top")
```
...
The results here further show that the bottom percentiles for diamonds are similar, but are very different in the top percentiles. As discussed earlier, the minimum and the maximum are all very similar, but the difference between the 25th and 75th percentile become even more stark here. 
...
```{r, solution_03e}
mpg %>% ggplot(mapping=aes(x=displ,y=hwy)) + geom_point() + geom_smooth(formula = y ~ x, method=lm, aes(color=drv))
```
...
```{r, solution_03f}
mpg %>% ggplot(mapping=aes(x=displ,y=hwy)) + geom_point() + geom_smooth(formula = y ~ x, method=lm, aes(group=drv))
```
...
#### SOLUTION
Despite only using 2 variables, 3 are displayed. It is not immediately clear why drv is split into 2 variables to me, but I would think it is significant to look at all the variables. 
