---
title: "CS 1675 Fall 2021 Homework: 11"
subtitle: "Assigned November 20, 2021; Due: December 3, 2021"
author: "Nicholas Zullo"
date: "Submission time: November 30, 2021 at 9:00PM EST"
...
```{r, read_prob_01_data}
url_01 <- 'https://raw.githubusercontent.com/jyurko/CS_1675_Fall_2021/main/HW/11/hw_11_prob_01_train.csv'
...
```{r, solution_01a}
prob_01_df %>% ggplot(mapping=aes(y=f, x=x)) + geom_point() + geom_line() 
```
...
url_load_dir <- 'https://github.com/jyurko/CS_1675_Fall_2021/blob/main/HW/11'
...
The beta matrix has dimensions of D x H, where D is the number of inputs and H is the number of hidden units. In this case, we have a 2x2 beta matrix, so there are 2 input rows. One of the inputs is X from the data, and the other input is a 1 for the bias. The alpha vector has 3 elements, because it has dimensions (H+1) x 1. There are 2 hidden units and 1 bias that are used. Each beta parameter corresponds to the weight of the hidden units. 
...
The trends match what is displayed in 1b. Hidden Unit 1 has a negative slope of -1.76 and an intercept very close to 0, and we see that here in the graph. Hidden Unit 2 has a slope of .46 and an intercept close to 0 again, which is again shown here.
...
I think the network did a really close job approximating the sin function. The peak and valley is not as well fit as the linear section in the middle between x=-1 and x=1 is. This is surprising to me because we have done no training, so the given parameters must have been tuned already.
...
viz_hidden_trend_wrt_x(nnet_hidden_2b$H, 
                       ...
                       "hidden non-linear units")
```
The trends are similar to the previous problem. Both Hidden Unit 2s have negative slope, but now there is a difference between Hidden Unit 1 as it is now a negative slope. Both functions interccept at similar points as before. Hidden Unit 1 is now not as steep as before, as the start and end of the line are not as high in value now. The trends are different due to new values for the beta parameters. I expect the model to have a different performance now.
### 2b)
...
The two models look nearly identical. The trends overlap everywhere along the curve.
### 2c)
...
No, despite having very different weights, the behavior of the model was identical. 
...
H_05 for 5a was turned into 1 for most of its domain after applying the transformation. For 5b, it follows the sigmoid curve closely, going between 1 and -1. The trends between the two models seem very different. A lot have different slopes and intercepts between them, such as H_04, which goes from negative to positive, and changes intercepts.
...
The 5 hidden unit model performs better. By having more weights available to tune, the model is better able to adapt the data. Adding more weights adds more complexity. To tune this, we could start with a large amount of hidden units, train the model, and see the performance. Then slowly decrease the number of hidden units until we find a less complex model that still performs well.
...
lm_sine_mod <- lm(f ~ x, data = prob_01_df)
...
The linear model has 2 parameters, the intercept and the slope. However, an h hidden unit neural network has H(D+1) + H + 1 parameters, in this case 5*2 + 5 + 1. 
...
The linear model would not be so accurate if there was noise present in the data. The neural network can generalize to more use cases than a simple sin function as well. 
...
```{r, read_prob_03_data}
url_03_train <- 'https://raw.githubusercontent.com/jyurko/CS_1675_Fall_2021/main/HW/11/hw_11_prob_03_train.csv'
...
```{r, solution_03a}
prob_03_lf %>% ggplot(mapping=aes(y=y, x=value)) + geom_point() + facet_wrap(~name)
```
...
tibble::tibble(
  ...
  theme(legend.position = "top") 
```
The two functions have similar trends, but the difference is that tanh goes down to -1 while logistic stops at 0. 
...
$$
SSE = \sum_{n=0}^N(\mathbf{y}_n - f_n)^2 \\
f = \alpha_0 + H*\alpha \\ 
H = g(X*\beta)
$$
...
my_neuralnet_sse <- function(theta, my_info)
{
  ...
  total_num_betas <- my_info$num_params-my_info$num_hidden-1# how many total betas are there???????
  ...
  aw <- a_all[2:length(a_all)]# output layer weights?????
  
  # calculate the linear predictors associated with
  # each hidden unit

  vals = calc_hidden_units(X, Bmat, my_info$transform_hidden)
  ...
  # calculate the response (the output layer)
  f <- calc_nnet_response(H, a_all)
  # calculate the SSE
  return(sum((my_info$y - f)^2))
}
```
...
The values are not the same. I am not surpised to see the values be different. Neural networks are not unimodal so different start guesses will converge to different values.
...
optim_fit_3_d$par
```
Again, the values are not the same. 
...
optim_fit_3_d$value
```
Model b and d have the lowest error of ~33, so they are the best.
...
train_1layer_nnet_sse <- function(num_hidden, transform_func, X, y)
{
  ...
  my_info_list$num_params <- num_hidden*ncol(X) + num_hidden + 1# total number of hidden and output layer parameters
...
```
Model 24_b has the lowest training error. The more parameters we add, the lower the SSE is. 
...
```{r, read_holdout_set}
url_03_test <- 'https://raw.githubusercontent.com/jyurko/CS_1675_Fall_2021/main/HW/11/hw_11_prob_03_test.csv'
...
{
  length_beta_per_unit <- ncol(X)# how many betas are there???????
  total_num_betas <- length(theta)-num_hidden-1# how many total betas are there???????
  ...
  vals = calc_hidden_units(X, Bmat, transform_func)
...
```{r, sol_5c}
assess_nnet_mse(optim_fit_3_a$par, 3, tanh, Xtest_03, prob_03_test_df$y)
assess_nnet_mse(optim_fit_3_b$par, 3, tanh, Xtest_03, prob_03_test_df$y)
...
Model 6a is best on the training set. It has the lowest MSE. This is different than the findings of the training set. We see that the higher parameter networks overfit the data.  