---
title: "CS 1675 Fall 2021 Homework: 05"
subtitle: "Assigned September 30, 2021; Due: October 7, 2021"
author: "Nicholas Zullo"
date: "Submission time: October 6, 2021 at 11:00PM EST"
...
```{r, sol_1b}
qnorm(p=c(.25,.75),mean=125, sd=5)

```
50% of the probability mass is within these 2 values.
...
N = \left(\frac{1}{\tau_N^2} -\frac{1}{\tau_0^2}\right) * \sigma^2
$$
```{r, sol_1d}
((1/2^2) - (1/25))*25
...
$$
\log(p(x | \mu, \sigma)) \propto \sum^N_{n=1} \log\left(\frac{1}{\sqrt{2\pi\sigma^2}}*\exp\left(-\frac{1}{2*\sigma^2}*(x_n-\mu)^2\right)\right) \\
\log(p(x | \mu, \sigma)) \propto \sum^N_{n=1} \log(1) - \log(\sqrt{2\pi\sigma^2}) - \frac{1}{2}\left(\log(2\pi) - \log(\sigma^2) - \frac{1}{2\sigma^2}(x_n-\mu)^2 \right) \\
\log(p(x | \mu, \sigma)) \propto \frac{-1}{2}\log(2\pi)\sum^N_{n=1}(1) - \frac{1}{2} \log(\sigma^2) \sum^N_{n=1}(1) + \sum^N_{n=1}(1)(\frac{-1}{2\sigma^2}(x_n-\mu)^2) \\
\log(p(x | \mu, \sigma)) \propto \sum^N_{n=1}\left(\frac{-1}{2\sigma^2}(x_n-\mu)^2\right)
$$
...
$$
d/d\mu \log(p(x | \mu, \sigma)) \propto \sum^N_{n=1} d/d\mu\left(\frac{-1}{2\sigma^2}(x_n-\mu)^2\right) \\
d/d\mu \log(p(x | \mu, \sigma)) \propto \sum^N_{n=1} \frac{1}{\sigma^2}\left(x_n-\mu\right) \\
d/d\mu \log(p(x | \mu, \sigma)) \propto \frac{N}{\sigma^2}\left(\bar{x} - \mu\right)
$$
...
$$
0 = \frac{N}{\sigma^2}\left(\bar{x} - \mu\right) \\
\bar{x} = \mu
$$
...
It is the same as the Bernoulli, where the MAP is equal to the average of the data.
...
$$
\log(p(\sigma | x,\mu)) \propto \sum^N_{n=1}\left( \log \left( \frac{1}{\sigma\sqrt{2\pi}} exp\left(\frac{-1}{2\sigma^2}(x_n-\mu)^2    \right) \right) \right) \log(\lambda*exp(-\lambda*\sigma)) \\
\log(p(\sigma | x,\mu)) \propto \sum^N_{n=1}\left(-\log(\sigma*\sqrt{2\pi})-\frac{1}{2\sigma^2}(x_n-\mu)^2        \right) \log(\lambda)-\lambda*\sigma  \\
\log(p(\sigma | x,\mu)) \propto -N\log(\sqrt{2\pi}\sigma) -\frac{1}{2\sigma^2}\sum^N_{n=1}\left((xn-\mu)^2 \right) -\lambda*\sigma 
$$
...
#### SOLUTION
$$
\log(p(\sigma | x,\mu)) \propto -N\log(2\pi\sigma) -\frac{N*v}{2\sigma^2}-\lambda\sigma
$$
...
{
  return(-N * log(sqrt(2*pi)*sigma) - (N*v)/(2*sigma^2) - lambda*sigma)
}
```
...
tibble::tibble(
  ...
  mutate(log_post=log_post_sigma_unnorm(sigma, N, v, .75)) %>%
  ggplot(mapping=aes(x=sigma, y=log_post)) +
  geom_line()
```

The shape does not really look like a parabola, although it appears to be a second order function. It looks to have only 1 peak, so it has only 1 mode. 
...
log_post_varphi_unnorm <- function(varphi, N, v, lambda)
{
  ...
  log_post <- (-N * log(sqrt(2*pi)*sigma) - (N*v)/(2*sigma^2) - lambda*sigma)
  
  # account for the derivative adjustment
  log_post <- log_post * log(sigma)
}
```
...
tibble::tibble(
  varphi = seq(from=log(.5), to=log(4.5), length.out=101)
) %>% 
  mutate(log_post=log_post_varphi_unnorm(varphi, N, v, .75)) %>%
...
What do you think?  
It does seem more parabolic now, but still is not a true parabola. 
...
tibble::tibble(
  x = seq(from=-.5, to=2, length.out=501)
) %>% 
  mutate(y=my_func(x,my_constants)) %>%
  ggplot(mapping = aes(x = x, y = y)) + 
  geom_line()
```
...
tibble::tibble(
  ...
  ggplot(mapping = aes(x = x, y = y)) + 
  geom_line() + 
  geom_vline(xintercept=opt_result$par)
```
...
**Use `optim()` to find the MAP of the log-transformed likelihood noise, $\varphi$, using an initial guess equal to -0.5. Assign all arguments to `optim()` that you feel are required to find the value that maximizes the log-posterior. Again, assign the `method='BFGS'` instead of `method='Brent'` as used in Problem 04.**  
...
varphi_opt_a <- optim(-.5, log_post_varphi_unnorm, gr=NULL, N, v, .75, method="BFGS",hessian=TRUE,control=list(fnscale=-1))
varphi_opt_a
```
...
```{r, 5b_sol}
varphi_opt_b <- optim(1.35, log_post_varphi_unnorm, gr=NULL, N, v, .75, method="BFGS", hessian=TRUE, control=list(fnscale=-1))
varphi_opt_b
```
Yes it is different. There is a local max that the function converged to that was different than when starting from -.5
### 5c)
