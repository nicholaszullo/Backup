---
title: "CS 1675 Fall 2021 Homework: 06"
subtitle: "Assigned October 7, 2021; Due: October 14, 2021"
author: "Nick Zullo"
date: "Submission time: October 14, 2021 at 8:40PM EST"
...
```{r, read_in_data}
hw06_data_url <- "https://raw.githubusercontent.com/jyurko/CS_1675_Fall_2021/main/HW/06/hw06_data.csv"
...
hw06_info <- list(
  ...
  mu_0 = 11,### mu_0 value
  tau_0 = .5,### tau_0 value
  sigma_rate = .5 ### rate (lambda) on sigma
)
```
...
sigma_grid_lwr <- 1
sigma_grid_upr <- qexp(p=.99,rate=hw06_info$sigma_rate)
```
...
param_grid %>% 
  ...
  geom_point(data = tibble::tibble(xbar = sample_mean, xsd = sample_std),
...
The prior mode is near the sample mean already. Also, there is a clear correlation between sigma and mu, causing the surface to be nonsymmetrical around the prior mode.
...
#### SOLUTION
This is given as the first derivative of the un-normalized log-posterior on $\mu$ in 2b of homework 5
$$
d/d\mu(\log(p(\mu | x, \sigma))) = \frac{N}{\sigma^2}(\bar x - \mu) - \frac{1}{\tau_0^2}(\mu-\mu_0)
$$

Now to take the second derivative.
$\bar x$ and $\mu_0$ are dropped in the derivative as they are constants that do not depend on $\mu$
$$
d/d\mu\left(\frac{N}{\sigma^2}(\bar x - \mu) - \frac{1}{\tau_0^2}(\mu-\mu_0)\right) = - \frac{N}{\sigma^2}-\frac{1}{\tau_0^2}
$$
### 2b)
...
The second derivative test is used to determine if a critical point, x, is a minimum or a maximum. If the second derivative at x is positive than x is a minimum. If the second derivative at x is negative than x is a maximum. 
...
#### SOLUTION
The posterior mode was given to be
$$
\mu_{MAP} = \frac{\frac{N}{\sigma^2}\bar x + \frac{1}{\tau_0^2}\mu_0}{\frac{N}{\sigma^2} + \frac{1}{\tau_0^2}} \\
\mathrm{m}_N = \mu_{MAP}
$$
Which for a normal distribution is the same as the mean.

The posterior standard deviation is
$$
covariance matrix = [\sigma^2] \\
\mathrm{s}_N = \sqrt{\sigma^2} = \sigma
$$
### 2d)
...
We do not need to approximate the mode here. We are able to directly calculate the mode, so this is not an approximation
...
We need to unbound $\sigma$. By transforming from $\sigma$ to $\varphi$ we are able to fully utilize the normal distribution. Since the normal distribution is not bounded, we do not want to have a bounded variable either.
...
$$
p \left( \mu, g(\sigma) \mid \mathbf{x} \right) \propto \prod_{n=1}^{N} \left( \mathrm{normal} \left(x_n \mid \mu, g(\sigma) \right) \right) \times \mathrm{normal}\left(\mu \mid \mu_0, \tau_0\right) \times \mathrm{Exp}\left(g(\sigma) \mid \lambda=0.5\right)
$$
...
cv_param_grid %>% 
  ...
  geom_point(data = tibble::tibble(xbar = sample_mean, log_xsd = log(sample_std)),
  ...
  theme_bw()
```
Here, the surface is much more symmetrical compared to earlier. The posterior mode is still close to the sample mean, and the sample mean is in a moderately high probability section of the surface. This is a good sign as the prior was a good estimate for the sample, and after training on a few data points the posterior will have no problem converging to the sample mean. 
## Problem 04
...
map_res_02$par
```
The optimal parameter is very very close to the same. This is a good sign, since the initial starting points were on opposite sides of the surface, that they still converged to nearly the same point.

```{r, sol_4c_2}
map_res_01$hessian
map_res_02$hessian
```
The hessian matrices are also nearly identical. Again this is a good sign, meaning that the optimal point was likely found correctly, and we did not reach 2 separate maximums.

Since the surface was symmetrical looking, it is no surprise that starting on opposite sides led to nearly identical results. If the surface was highly sloped in one direction, we might have seen different results. 
### 4d)
...
laplace_result <-  my_laplace(c(sample_mean,log(sample_std)), my_cv_logpost, hw06_info)
laplace_result$converge#The solution converged
laplace_result$mode[1]#Mean of \mu
laplace_result$mode[2]#Mean of \varphi
sqrt(laplace_result$var_matrix[1])#diagonal elements x are what we want [[x,a],[b,x]]
sqrt(laplace_result$var_matrix[4])#Standard deviations are diagonal elements 
sqrt(laplace_result$var_matrix[1])/sqrt(laplace_result$var_matrix[4])#correlation is ratio between \mu and \varphi
```
...
```{r, sol_5a}
(laplace_result$mode[1] - sample_mean) / sqrt(laplace_result$var_matrix[1])
```
The sample mean is 1.4 standard deviations below the unknown mean. So using Z-Score, the probability the unknown mean is below the sample mean is 1-.9192 =.0808, so 8.08% chance.
### 5b)
...
post_samples <- post_samples %>% mutate(inverse_g=exp(varphi)) 
mean(post_samples$inverse_g) == mean(post_samples$sigma)
```

The results are equal. We have successfully transformed back to sigma
...
```
The distribution on sigma does not look gaussian. This is to be expected because we used an Exp() distribution to model the noise, not normal. We used a normal to model \mu, and it does look normal
...
```{r, sol_5e}
quantile(post_samples$sigma, .8916048)
```
Since 89.16048% of the data is below 4, that means 10.83952% of the data is above 4. So the probability is .1083952
