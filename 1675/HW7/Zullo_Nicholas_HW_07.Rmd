---
title: "CS 1675 Fall 2021 Homework: 07"
...
url_01 <- "https://raw.githubusercontent.com/jyurko/CS_1675_Fall_2021/main/HW/07/train_01.csv"
...
```{r, solution_01a}
train_01 %>% ggplot(mapping=aes(x=x, y=y)) + geom_point() + geom_smooth(method="lm")
```
The data appears to follow a linear relationship very closely
...
```{r, solution_01c_c}
lm_logpost(unknowns = rep(-1, length.out = 51), my_info = info_01)
```
...
```{r, solution_01c_b}
lm_logpost(unknowns = rep(1, length.out= 21), my_info = info_01)
```
...
my_laplace <- function(start_guess, logpost_func, ...)
{
  ...
  int <- p/2 * log(2*pi) + .5 * log(det(post_var_matrix)) + logpost_func(mode, ...)
...
laplace_01 <- my_laplace(rep(0, length.out= ncol(Xmat_01)+1), lm_logpost, info_01 )
```
...
```
We should not be concerned with the initial guess affecting our results. There is still only 1 max value because we are using a linear function. 
### 1e)
...
set.seed(87123)
post_samples_01 <- generate_lm_post_samples(mvn_result = laplace_01, length_beta=ncol(Xmat_01), num_samples = 2500)
```
...
```
The number of columns corresponds to S, the number of beta parameters to use
...
We are not using the predicted response values because we are interested in the variability between different values of beta. Later when we use all 2500 (S) posterior samples to find the error we use those values to find the best model.
...
We are taking the average over all data points for a given set of beta parameters. They MAE values are different because we are using different values for beta for each calculation.
...
abs(MAE_01[1] - mae_01_post_01) < .0000001
abs(MAE_01[2] - mae_01_post_02) < .0000001
abs(MAE_01[3] - mae_01_post_03) < .0000001
```
Since the values are equal, we have calculated the MAE correctly. 
...
The mean trend is uncertain because of uncertainty in the beta parameters, and the performance metrics are uncertain because of uncertainty in the mean trend. This means that the uncertainty in the betas is creating uncertainty in the MAE.
...
url_02_train <- "https://raw.githubusercontent.com/jyurko/CS_1675_Fall_2021/main/HW/07/train_02.csv"

train_02 <- readr::read_csv(url_02_train, col_names = TRUE)

url_02_test <- "https://raw.githubusercontent.com/jyurko/CS_1675_Fall_2021/main/HW/07/test_02.csv"
...
```{r solution_03a}
ggplot() + geom_point(data=train_02, mapping=aes(x=x,y=y)) + geom_point(data=test_02, mapping=aes(x=x, y=y), color="red")
```
...
make_spline_basis_mats <- function(J, train_data, test_data)
{
  ...
  train_matrix <- model.matrix(~ splines::ns(x, knots = knots_use_basis, df=J), data = train_data)
  
  test_matrix <-  model.matrix(~ splines::ns(x, knots = knots_use_basis, df=J), data = test_data)
...
```
```{r, fasd}
  train_basis <- splines::ns(train_02$x, df=4)
```
...
There are 150 rows in the train matrix and 50 rows in the test matrix.
...
manage_spline_fit <- function(Xtrain, logpost_func, my_settings)
{
  ...
  init_beta <- rnorm(ncol(Xtrain), 0, 1)
  
  init_varphi <- log(rexp(1, my_settings$sigma_rate))
  
  my_laplace(c(init_beta, init_varphi), lm_logpost, my_settings)
}
```
...
calc_mae_from_laplace <- function(mvn_result, Xtrain, Xtest, y_train, y_test, num_samples)
{
  ...
  post <- generate_lm_post_samples(mvn_result = mvn_result, ncol(Xtrain), num_samples)
...
all_spline_mae_results %>% ggplot(mapping=aes(as.factor(J), y=mae)) + geom_boxplot(mapping=aes(fill=dataset)) + scale_fill_brewer(palette="Set1")
```
...
all_spline_mae_results %>% filter(J > 3) %>% ggplot(mapping = aes(x=J, y=mae)) + stat_summary(geom = "line", fun="median") + facet_wrap(~dataset)
```

The models that appear to overfit are everything from 10 and above. The test error is no longer decreasing. This is a tell tale sign to stop training as you are now overfitting. The models continue to learn as the training error continues to decrease, but we see no benefit in the test error. 
...
Models 7 and 9 are very clearly the top choices, with 7 being slightly better.
...
This is similar to the results from the split comparison. The minimum test error occurs near 7 and 9 before it increases again. 
