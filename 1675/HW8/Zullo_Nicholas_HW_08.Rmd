---
title: "CS 1675 Fall 2021 Homework: 08"
subtitle: "Assigned October 29, 2021; Due: November 4, 2021"
author: "Nicholas Zullo"
date: "Submission time: November 4, 2021 at 1:15PM EST"
...
```{r, read_data}
data_url <- 'https://raw.githubusercontent.com/jyurko/CS_1675_Fall_2021/main/HW/08/hw08_prob01.csv'
...
```{r, solution_01a}
df %>% ggplot() + geom_point(mapping = aes(x = x1, y=y)) + geom_point(mapping=aes(x=x2, y=y), color="red") 
```
There is not an obvious trend here. It could be quadratic with a lot of noise. 
### 1b)
...
coefplot(mod01)
```
None of the features are significant because they all have 0 in the uncertainty range
...
coefplot(mod02)
```
We now have 3 features instead of 2. This time, the feature for x1 and x1:x2 are significant because they do not contain 0 in the uncertainty interval, but x2 still is not significant.
...
coefplot(mod03)
```
Here, we have 6 features present, and only x2^2 is statistically significant. All other features contain 0 in the uncertainty window
### 1e)
...
coefplot(mod04)
```
We do not have any significant features now
...
coefplot(mod05)
```
We have 3 statistically significant features here.
...
coefplot(mod06)
```
None of the features here are significant.
...
broom::glance(mod06)
```
The 6th model is the best according to R^2. The best AIC model is 3rd one. The best BIC model is the 3rd one.
...
I do feel the predictions are consistent. The complicated models severely overfit the data. These are models 4, 5, and 6. The graphs appear to connect the dots between the points, and do not learn a generic trend. The ribbon intervals are also much wider for these 3 models compared to models 1, 2, and 3. The models with the worst AIC/BIC are 5 and 6, and the defining characteristic is the complexity of the model. They are penalized for having too many parameters which causes it to overfit the data.
...
```{r, solution_03c}
laplace_03_weak <- my_laplace(rep(0, length.out= ncol(X03)+1) , lm_logpost,info_03_weak)
laplace_06_weak <- my_laplace(rep(0, length.out= ncol(X06)+1) , lm_logpost,info_06_weak)
...
```{r, solution_03e}
exp(laplace_03_weak$log_evidence)/(exp(laplace_03_weak$log_evidence) + exp(laplace_06_weak$log_evidence))
exp(laplace_06_weak$log_evidence)/(exp(laplace_03_weak$log_evidence) + exp(laplace_06_weak$log_evidence))
```
The Bayes factor says that model 3 is the best model.
...
```{r, solution_03f_b}
laplace_03_strong <- my_laplace(rep(0, length.out= ncol(X03)+1) , lm_logpost,info_03_strong)
laplace_06_strong <- my_laplace(rep(0, length.out= ncol(X06)+1) , lm_logpost,info_06_strong)
...
```{r, solution_03h_b}
laplace_03_very_strong <- my_laplace(rep(0, length.out= ncol(X03)+1) , lm_logpost,info_03_very_strong)
laplace_06_very_strong <- my_laplace(rep(0, length.out= ncol(X06)+1) , lm_logpost,info_06_very_strong)
...
The coefficient values get smaller the stronger the prior is. With a very small prior standard deviation, the coefficients are basically all 0. With the strong prior, and a more reasonable sd of 1, the coefficients are all small values but not too small. This prior standard deviation works well to prevent extreme values of the coefficients. With a very weak prior and a prior standard deviation of 50, the coefficient values are allowed to vary to large extremes, which allows the model to overfit. 
...
```{r, solution_03k}
exp(laplace_03_strong$log_evidence)/(exp(laplace_03_strong$log_evidence) + exp(laplace_06_strong$log_evidence))
exp(laplace_06_strong$log_evidence)/(exp(laplace_03_strong$log_evidence) + exp(laplace_06_strong$log_evidence))
```
```{r, solution_03k_b}
exp(laplace_03_very_strong$log_evidence)/(exp(laplace_03_very_strong$log_evidence) + exp(laplace_06_very_strong$log_evidence))
exp(laplace_06_very_strong$log_evidence)/(exp(laplace_03_very_strong$log_evidence) + exp(laplace_06_very_strong$log_evidence))
```
The prior does influence which model is considered better. The 3rd model is correctly identified as the top when using the strong prior, but the Bayes Factor is much less sure about it when using the very stron prior. With the strong and weak, the Bayes factor is 1 for the 3rd model, which clearly means it is the best. But with the very strong, there is less certainty because it is only .73
## Problem 04
...
post_pred_summary_viz_03_weak %>% 
  ...
  geom_line(mapping=aes(y=mu_avg), color="red") + 
...
```{r, solution_04e}
X06_test <- model.matrix(~ splines::ns(x1, knots = make_splines_training_knots(12, viz_grid, "x1"), df=12):(x2+I(x2^2)+I(x2^3)+I(x2^4)), data=viz_grid)
...
The predictions follow a similar trend as the coefficients. The strong models are the best out of weak, strong, and very strong. The 3rd model creates much better predictions than the 6th model, and 3rd model strong is the best overall. 
...
X06_glmnet <- model.matrix(y~ splines::ns(x1, knots = make_splines_training_knots(12, df, "x1"), df=12):(x2+I(x2^2)+I(x2^3)+I(x2^4)) -1, data=df)
```
...
```
With model 3, we are down to 1 coefficient by the end. With model 6, we are down to 2 coefficients by the end. However, we will select the model within 1 standard error of the lowest MSE. These models are different than the ones reached at the end. 
### 5f)
...
```
Model 3 has only 1 feature which is not turned off, and model 6 has 11 features not turned off. These models are different from each other 