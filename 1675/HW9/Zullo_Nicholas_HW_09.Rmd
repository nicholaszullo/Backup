...
date: "Submission time: November 12, 2021 at 5:10PM EST"
...
```{r, solution_01a}
df %>% ggplot() + 
  geom_point(mapping=aes(y=y, x=x1), color="red") +  
  geom_point(mapping=aes(y=y, x=x2), color="orange") +  
  geom_point(mapping=aes(y=y, x=x3), color="yellow") +  
  geom_point(mapping=aes(y=y, x=x4), color="green") +  
  geom_point(mapping=aes(y=y, x=x5), color="blue") +  
  geom_point(mapping=aes(y=y, x=x6), color="brown")   
```
There is not an obvious trend between the data. Most of the data seems to be centered around y=0 and x=0. 
...
```{r, solution_01c1}
df %>% select(-y) %>% cor() %>% corrplot::corrplot()
```


```{r, solution_01c2}
df %>% select(-y) %>% cor() %>% corrplot::corrplot(type="upper")
```
...
```{r, solution_01d}
mat <- model.matrix(~ x1 + x2 + x3 + x4 + x5 + x6, data = df)
SSmat <- t(mat) %*% mat
covmat <- cov2cor(solve(SSmat))
covmat
corrplot::corrplot(covmat)
```
Much fewer relationships exist now, but the ones that do are much stronger. 
...
```{r, solution_01f}
SStrips <- t(Xtrips) %*% Xtrips
covtrips <- cov2cor(solve(SStrips))
corrplot::corrplot(covtrips, type="upper", method="square")
```
...
```{r, solution_2c}
laplace_weak_ridge <- my_laplace(rep(0, length.out = ncol(Xtrips)+1), bayes_ridge_logpost, info_weak_ridge)
laplace_strong_ridge <- my_laplace(rep(0, length.out = ncol(Xtrips)+1), bayes_ridge_logpost, info_strong_ridge)
laplace_very_strong_ridge <- my_laplace(rep(0, length.out = ncol(Xtrips)+1), bayes_ridge_logpost, info_very_strong_ridge)
```
...
```{r, solution_02e_1}
laplace_weak_ridge$var_matrix %>% cov2cor() %>% corrplot::corrplot(type="upper", method="square")
```


```{r, solution_02e_2}
laplace_strong_ridge$var_matrix %>% cov2cor() %>% corrplot::corrplot(type="upper", method="square")
```


```{r, solution_02e_3}
laplace_very_strong_ridge$var_matrix %>% cov2cor() %>% corrplot::corrplot(type="upper", method="square")
```
...
The stronger the prior, the less correlation we see in the posterior. Both the strong and weak prior have 2 statistically significant features, x1:x4 for both, x2:x3 for weak and x5:x6 for strong. The very strong prior does not have any statistically significant features. The correlation structure is similar between the strong and weak prior. The strongly correlated features in the weak prior are also strongly correlated in the strong prior. Features in the weak prior that are not strongly correlated drop to 0 correlation in the strong prior, but the main structure of strong correlations remains. 
...
```{r,sol_03_test}
double_exp_density(c(-1,-.5,-.25),0,1)
```
...
The variance of the double exponential is given in regularization slide 80 to be $2b^2$. This means that 
$$
\tau_{\beta}^2 = 2b^2
$$
The standard deviations are therefore
$$
\sqrt{\tau_{\beta}^2 = 2b^2} \\
\tau_{\beta} = \sqrt{2}b
$$
Solving for b:
$$
\frac{\tau_{\beta}}{\sqrt{2}} = b
$$
### 3c)
...
#### SOLUTION
standard normal has $\tau_{\beta}=1$, so $b=1/2$

```{r, solution_03c}
tibble::tibble(
  x = seq(-3,3, length.out=101)
) %>% mutate(y=double_exp_density(x, -0, 1/sqrt(2)))%>%
...
info_weak_lasso <- list(
  ...
  design_matrix = Xtrips,
  mu_beta = 0,
  b_beta = (info_weak_ridge$tau_beta)/sqrt(2),
  sigma_rate = 1 
)
```
...
info_strong_lasso <- list(
  ...
  design_matrix = Xtrips,
  mu_beta = 0,
  b_beta = (info_strong_ridge$tau_beta)/sqrt(2),
  sigma_rate = 1 
)
```
...
info_very_strong_lasso <- list(
  ...
  design_matrix = Xtrips,
  mu_beta = 0,
  b_beta = (info_very_strong_ridge$tau_beta)/sqrt(2),
  sigma_rate = 1 
)
```
...
bayes_lasso_logpost <- function(unknowns, my_info)
{
  ...
  # evaluate the log-likelihood
  log_lik <- sum(dnorm(x = my_info$yobs, mean = mu, sd = lik_sigma, log = TRUE))
    #sum(dnorm(x = my_info$yobs, mean = mu, sd = lik_sigma, log = TRUE))
    #-1/(2*lik_sigma^2)* (t(my_info$yobs - X %*% beta_v) %*% (my_info$yobs - X %*% beta_v))
  # evaluate the log-scake
  log_prior_beta <- sum(log(double_exp_density(x = beta_v, mu = my_info$mu_beta, b = my_info$b_beta)))
...
Test the function with a guess of -2.2 for all parameters and the weak lasso prior.  

```{r, solution_03e_b}
bayes_lasso_logpost(rep(-2.2, length.out=ncol(Xtrips)+1), info_weak_lasso)
```
...
```{r, solution_03f}
laplace_weak_lasso <- my_laplace(rep(0,length.out=ncol(Xtrips)+1), bayes_lasso_logpost, info_weak_lasso)
laplace_strong_lasso <- my_laplace(rep(0,length.out=ncol(Xtrips)+1), bayes_lasso_logpost, info_strong_lasso)
laplace_very_strong_lasso <- my_laplace(rep(0,length.out=ncol(Xtrips)+1), bayes_lasso_logpost, info_very_strong_lasso)
```
...
Many more parameters have means at 0 and a very small (if any) uncertainty range. This makes sense, as these are parameters that lasso has turned off. This is especially noticable when comparing the strong ridge graph with the strong lasso graph. Nearly all the triple interaction parameters get turned to 0 in lasso strong, while they still have variation and uncertainty in the ridge strong. The very strong graphs are both all 0s and there is no difference here. The weak priors have slightly smaller uncertainty ranges for a few values near 0, but most remain the same as ridge.
...
The Laplace approximation is used when the posterior has a Gaussian distribution. when we use the double exponential, the posterior distribution is no longer Gaussian and the Laplace approximation cannot truely be applied.
...
```{r, solution_04b}
enet_default <- train(y ~ .^3, data = df, method="glmnet", metric=my_metric, preProcess = c("center", "scale"), trControl=my_ctrl)
enet_default
```

What do you think?  
The model with the lowest errors used alpha=1 and lambda = .008321. Using an alpha of 1 means that we are using lasso exclusively. 
...
enet_grid <- expand.grid(
  alpha = seq(.1, .9, length.out=9),
  lambda = exp(seq(log(min(enet_default$results$lambda)), log(max(enet_default$results$lambda)), length.out=25))
)
```
There are 9 * 25 tuning combinations, which is 225 in total. Each model is fit with each combination of tuning parameters. Since we resample 25 times, we are fitting 25 * 225 = 5625 models.
...
```{r, solution_04d}
enet_tune <- train(y ~ .^3, data = df, method="glmnet", metric=my_metric, preProcess = c("center", "scale"), trControl=my_ctrl, tuneGrid=enet_grid)
plot(enet_tune, xTrans=log)
enet_tune$bestTune
```

The model is most similar to lasso, since the optimal alpha is found to be .9.
...
What do you think?  
The high interaction variables are not very important, as most of them get turned off by the model. Most of the linear variables remain, so those are more significant than the others. This does seem to be consistent with our earlier strong models. 
### 4f)
...
```{r, solution_4f}
plot(varImp(enet_tune, xTrans=log))
```


What do you think?  
This is consistent with what we found in the pervious section. The triple interactions have very little importance. The linear variables have much more importance. The top variable is x5:x6, which is one of the non zero variables found by lasso in problem 3. 