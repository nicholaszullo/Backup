...
def shakespeare_writer():
  ...
  while len(generated_chars) < FLAGS.generation_length:
    outputs, _ = model(history)
    preds = sample_next_char_id(outputs)

    generated_chars.append(index2char[preds.item()])
    preds = preds.reshape((1,1))
    history = torch.cat((history,preds), 1)

    if history.shape[1] > FLAGS.history_length:
      history = history[:,history.shape[1] - FLAGS.history_length :]

  final = start_string + ''.join(generated_chars)
  print(final)
  return final
