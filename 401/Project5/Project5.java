...
public class Project5
{
	...
	{	
		if (n == 0)					//Exit condition, 1 not 0 becuase x^0 is 1 
			return 1;
	...
	{	
		...
		if (i == count-1) 			//Exit condition, before going out of bounds
			return true;
		if (array[i] < array[i+1])
			return true && isSorted(array, i+1, count);		//if it finds an element that is false it will start only returning false
		
		return false;										//adds a false into the && iff an element is out of order
	}
	...
	{	
		...
		if (lo > hi)			//Exit condition, once all letters are checked (lo==hi wont check middle letter if odd length)
			return true;
		if (s.charAt(lo) == s.charAt(hi))
			return true && isPalindrome(s, lo+1, hi-1);
