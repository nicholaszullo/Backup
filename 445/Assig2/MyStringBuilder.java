...
public class MyStringBuilder
{
	...
	public MyStringBuilder(char [] s)								//Same as String
	{
		...
		length = 1;
		CNode currNode = firstC;
		for (int i = 1; i < s.length; i++){							//Creates new nodes and adds them to the list
	...
	{
		firstC = null;												//Initialize all values to base case
		lastC = null;
		length = 0;
	}
	...
	public MyStringBuilder delete(int start, int end)
	{
		if (start >= length || end <= start)						//If parameters are invailid, return unmodified MSB
			return this;
		if (end > length)											//If removing past the end, only remove up until the end to not go out of bounds
			end = length;
	...
	public MyStringBuilder deleteCharAt(int index)
	{
		...
		for (int i = 0; i <= index; i++){
			if (i == index-1)
				startC = currNode;									//Finds the node before deletion and the one after deletion
			if (i == index)
				endC = currNode.next;
	...
	public MyStringBuilder insert(int offset, String str)
	{
		...
		CNode currNode = firstC;
		for (int i = 0; i < offset-1; i++){							//Find node offset begins
			currNode = currNode.next;
		}
		MyStringBuilder other = new MyStringBuilder(str);
		
		if (offset == 0){											//Special case if offset is 0 
	...
	public MyStringBuilder insert(int offset, char c)
	{
		if (offset > length)										//If offset is bigger than length, do nothing
			return this;
		CNode currNode = firstC;
		for (int i = 0; i < offset-1; i++){							//Find where offset starts
			currNode = currNode.next;
		}
		CNode other = new CNode(c);
		if (offset == 0){											//Special case insert to front, same as append 0
			other.next = firstC; 
			firstC = other;	
		}else {
	...
	public MyStringBuilder insert(int offset, char [] c)
	{
		...
		CNode currNode = firstC;
		for (int i = 0; i < offset-1; i++){							//Find offset
			currNode = currNode.next;
		}
		
		MyStringBuilder other = new MyStringBuilder(c);
		
		if (offset == 0){											//Special case adding to beginning
			other.lastC.next = firstC; 
			firstC = other.firstC;	
		}else {
			other.lastC.next = currNode.next;						//Link end of new string to rest of original string
	...
	{
		if (start < 0 || end <= start)								//Ensure inputs are valid
			return this;
		if (end > length)
			end = length;
		...
		if (start == 0){											//2 special cases
			...
			startC.next = other.firstC;								//Link string into new location in this
	...
	public String substring(int start, int end)
	{
		...
		return new String(temp);									//Make the char[] into a string to return
	}
	...
	public String toString()
	{
		...
		int i = 0;
		CNode currNode = firstC;									//Same idea as substring but the entire list not sections of it
		while (currNode != null)
		{
