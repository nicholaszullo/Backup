...
#define UPDATE_USED(x, y){\
			       x->sizeAndTags = x->sizeAndTags | (y);\
			       *(size_t*)UNSCALED_POINTER_ADD(x,SIZE(x->sizeAndTags)-WORD_SIZE) = x->sizeAndTags;\
			  }
#define FREE_TAGS(x, y){\
			   x->sizeAndTags = x->sizeAndTags ^ (y);\
                           *(size_t*)UNSCALED_POINTER_ADD(x,SIZE(x->sizeAndTags)-WORD_SIZE) = x->sizeAndTags;\
                      } 
...
static BlockInfo* searchFreeList(size_t reqSize) {
  BlockInfo* freeBlock = FREE_LIST_HEAD;
  int found = 0;
  while (!found)
  {
     if (freeBlock == NULL)
     {
        requestMoreSpace(reqSize);				//If no Block is big enough, get one with enough size
	freeBlock = FREE_LIST_HEAD;				//New block is inserted at head so move cursor there
     }
     if (SIZE(freeBlock->sizeAndTags) == reqSize)		//Exact match found
     { 	
	BlockInfo * next = UNSCALED_POINTER_ADD(freeBlock,reqSize);
	next->prev = freeBlock->prev;
	UPDATE_USED(next, TAG_PRECEDING_USED);			//Update the boundry tags
	UPDATE_USED(freeBlock, TAG_USED);
	return freeBlock;
     }
     else if (SIZE(freeBlock->sizeAndTags) > reqSize)		//A match is found
     {
		
	if (SIZE(freeBlock->sizeAndTags)-reqSize < MIN_BLOCK_SIZE)		//If after a split the block would be too small, don't split, treat like exact match
	{
	  BlockInfo * next = UNSCALED_POINTER_ADD(freeBlock, SIZE(freeBlock->sizeAndTags));
	  next->prev = freeBlock->prev;
	  UPDATE_USED(next, TAG_PRECEDING_USED);  
	  UPDATE_USED(freeBlock, TAG_USED);
	  return freeBlock;
	}
	
	BlockInfo* split = UNSCALED_POINTER_ADD(freeBlock, reqSize);			//The new block is right after the old block
	split->sizeAndTags = (SIZE(freeBlock->sizeAndTags) - reqSize);			//Transfer values to new block
	*(size_t*)UNSCALED_POINTER_ADD(split,SIZE(split->sizeAndTags)-WORD_SIZE) = split->sizeAndTags;		//Update footer
	split->next = freeBlock->next;
	split->prev = freeBlock->prev;
	freeBlock->sizeAndTags = reqSize | (freeBlock->sizeAndTags & 2);						//Update old block retaining preceding tag
	*(size_t*)UNSCALED_POINTER_ADD(freeBlock,SIZE(freeBlock->sizeAndTags)-WORD_SIZE) = freeBlock->sizeAndTags;	//Update footer
	if (freeBlock->next != NULL)
        {
	   freeBlock->next->prev = split;			//Prev of next block is now split not freeBlock, unless split is the end
	}
	freeBlock->next = split;	
	UPDATE_USED(freeBlock, TAG_USED);			//Update boundry tags
	UPDATE_USED(split, TAG_PRECEDING_USED);
	coalesceFreeBlock(split);
	return freeBlock;
     }
     
     freeBlock = freeBlock->next;				//Move to next block in list
...
static void insertFreeBlock(BlockInfo* freeBlock) {
  ...
  if (oldHead != NULL)						//If list is empty, dont update head block becuse it doesnt exist
  {	
    oldHead->prev = freeBlock;					//Link new block before head
  }

  freeBlock->next = oldHead;					//Link new block into head
  freeBlock->prev = NULL;
  
  FREE_LIST_HEAD = freeBlock;					//Update head reference
...
static void removeFreeBlock(BlockInfo* freeBlock) {
  ...
  {
    FREE_LIST_HEAD = NULL;					//If removing the only block, set the head to null
  } 
  else if (prevFree == NULL)					//If at the begining, the head is the next block
  {
    nextFree->prev = NULL;
    FREE_LIST_HEAD = nextFree;
  }
  else if (nextFree == NULL)					//IF at the end, the previous is now the end
  {
    prevFree->next = NULL;
  } 
  else 
  { 
    prevFree->next = nextFree;					//Regular case link passed current block
...
static void coalesceFreeBlock(BlockInfo* oldBlock) {
  ...
  if (newSize != oldSize) {
    ...
    *(size_t*)UNSCALED_POINTER_SUB(blockCursor, WORD_SIZE) = newSize | TAG_PRECEDING_USED;
	//*((size_t*)((void*)newBlock)+SIZE(newBlock->sizeAndTags)-sizeof(size_t)) = newBlock->sizeAndTags | TAG_PRECEDING_USED;
...
//  BlockInfo * next = NULL;
  ...
  ptrFreeBlock = searchFreeList(reqSize);					//Find the free block
  removeFreeBlock(ptrFreeBlock);						//Remove it from freeList
  return UNSCALED_POINTER_ADD(ptrFreeBlock, WORD_SIZE);				//return it
...
void mm_free(void *ptr) {
  ...
  if (ptr == NULL)								//Cant free a null pointer
  {     
     return ;
  }
  ...
  if ((blockInfo->sizeAndTags & 1) != 1)					//If freeing a block not marked used do nothing
  { 
     printf("%p NOT ALLOCATED!\n", ptr);
     return ;
  }  
   else 
  {
     FREE_TAGS(blockInfo, TAG_USED);						//Update tags of blocks
     FREE_TAGS(followingBlock, TAG_PRECEDING_USED);
  }
  insertFreeBlock(blockInfo);							//Insert the new block into the free list
  coalesceFreeBlock(blockInfo);							//Coalesce if needed

}

// Implement a heap consistency checker as needed.
int mm_check() {								//Prints all the blocks linked by the free list and info about them
 
  BlockInfo* block = FREE_LIST_HEAD;
  printf("head = %p\n", block);
  for (; block != NULL; block = block->next) {
      printf("address %p size %ld used %ld prev used %ld footer %ld\n",block, SIZE(block->sizeAndTags), block->sizeAndTags & 1, block->sizeAndTags & 2,  *(size_t*)UNSCALED_POINTER_ADD(block,SIZE(block->sizeAndTags)-WORD_SIZE)); 
  }
  return 0;
}
